    ...
    <footer className="bg-skin-card border-t border-skin-border mt-16">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row items-start justify-between gap-6 mb-6">
          ...
          {/* Quick links */}
          <div className="flex flex-wrap gap-4 text-xs text-skin-muted">
            {[
          ...
          <div className="flex items-center gap-1 bg-skin-control-bg rounded-xl p-1">
            {([['light', <Sun size={13} />, 'روشن'], ['dark', <Moon size={13} />, 'تاریک'], ['reading', <Book size={13} />, 'مطالعه']] as [Theme, React.ReactNode, string][]).map(([t, icon, label]) => (
              <button
                ...
                title={label}
                aria-label={label}
                className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-medium transition-all ${theme === t ? 'bg-skin-card text-skin-primary shadow-sm' : 'text-skin-muted hover:text-skin-text'}`}
              >
                {icon}
              </button>
            ))}
      ...
      >
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 mr-auto">
            ...
            <button
              onClick={enterAdmin}
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 text-xs text-skin-muted hover:text-skin-primary bg-skin-control-bg hover:bg-skin-control-hover rounded-xl transition-all font-medium"
            >
