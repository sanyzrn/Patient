...
( function () {
	...
	var ICON = {
		message: function ( s ) {
			return svg( '<path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/>', s ); },
		x: function ( s ) {
			return svg( '<path d="M18 6 6 18"/><path d="m6 6 12 12"/>', s ); },
		bot: function ( s ) {
			return svg( '<path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/>', s ); },
		send: function ( s ) {
			return svg( '<path d="M14.536 21.686a.5.5 0 0 0 .937-.024l6.5-19a.496.496 0 0 0-.635-.635l-19 6.5a.5.5 0 0 0-.024.937l7.93 3.18a2 2 0 0 1 1.112 1.11z"/><path d="m21.854 2.147-10.94 10.939"/>', s ); },
		loader: function ( s ) {
			return svg( '<path d="M21 12a9 9 0 1 1-6.219-8.56"/>', s ); },
		sparkles: function ( s ) {
			return svg( '<path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"/>', s ); },
		user: function ( s ) {
			return svg( '<path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>', s ); },
		package: function ( s ) {
			return svg( '<path d="M11 21.73a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73z"/><path d="M3.3 7 12 12l8.7-5"/><path d="M12 22V12"/>', s ); },
		chevronLeft: function ( s ) {
			return svg( '<path d="m15 18-6-6 6-6"/>', s ); },
		arrowRight: function ( s ) {
			return svg( '<path d="M5 12h14"/><path d="m12 5 7 7-7 7"/>', s ); },
		building: function ( s ) {
			return svg( '<rect width="16" height="20" x="4" y="2" rx="2" ry="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/><path d="M16 10h.01"/><path d="M16 14h.01"/><path d="M8 10h.01"/><path d="M8 14h.01"/>', s ); },
		activity: function ( s ) {
			return svg( '<path d="M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.49 12H2"/>', s ); },
		headphones: function ( s ) {
			return svg( '<path d="M3 14h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2zm0 0a9 9 0 0 1 18 0m0 0v3a2 2 0 0 1-2 2h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2z"/>', s ); },
		check: function ( s ) {
			return svg( '<path d="M21.801 10A10 10 0 1 1 17 3.335"/><path d="m9 11 3 3L22 4"/>', s ); },
		phone: function ( s ) {
			return svg( '<path d="M13.832 16.568a1 1 0 0 0 1.213-.303l.355-.465A2 2 0 0 1 17 15h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2A18 18 0 0 1 2 4a2 2 0 0 1 2-2h3a2 2 0 0 1 2 2v3a2 2 0 0 1-.8 1.6l-.468.351a1 1 0 0 0-.292 1.233 14 14 0 0 0 6.392 6.384"/>', s ); },
		fileText: function ( s ) {
			return svg( '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7z"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10 9H8"/><path d="M16 13H8"/><path d="M16 17H8"/>', s ); },
		refresh: function ( s ) {
			return svg( '<path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M3 21v-5h5"/>', s ); },
		mic: function ( s ) {
			return svg( '<path d="M12 19v3"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><rect x="9" y="2" width="6" height="13" rx="3"/>', s ); },
		volume: function ( s ) {
			return svg( '<path d="M11 4.702a.705.705 0 0 0-1.203-.498L6.413 7.587A1.4 1.4 0 0 1 5.416 8H3a1 1 0 0 0-1 1v6a1 1 0 0 0 1 1h2.416a1.4 1.4 0 0 1 .997.413l3.383 3.384A.705.705 0 0 0 11 19.298z"/><path d="M16 9a5 5 0 0 1 0 6"/><path d="M19.364 18.364a9 9 0 0 0 0-12.728"/>', s ); },
		stopCircle: function ( s ) {
			return svg( '<circle cx="12" cy="12" r="10"/><rect x="9" y="9" width="6" height="6" rx="1"/>', s ); },
		star: function ( s ) {
			return svg( '<path d="M11.525 2.295a.53.53 0 0 1 .95 0l2.31 4.679a2.12 2.12 0 0 0 1.595 1.16l5.166.756a.53.53 0 0 1 .294.904l-3.736 3.638a2.12 2.12 0 0 0-.611 1.878l.882 5.14a.53.53 0 0 1-.771.56l-4.618-2.428a2.12 2.12 0 0 0-1.973 0L6.69 21.01a.53.53 0 0 1-.77-.56l.881-5.139a2.12 2.12 0 0 0-.611-1.879L2.453 9.795a.53.53 0 0 1 .294-.906l5.165-.755a2.12 2.12 0 0 0 1.597-1.16z"/>', s ); },
		copy: function ( s ) {
			return svg( '<rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/>', s ); },
		alert: function ( s ) {
			return svg( '<path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3"/><path d="M12 9v4"/><path d="M12 17h.01"/>', s ); }
	};

	/* ---------- پیکربندی ---------- */
	var products     = ( cfg.products || [] ).filter(
		function ( p ) {
			return p && p.id; }
	);
	var companyInfo  = { id: cfg.companyId || 'nafas', name: cfg.companyName || '' };
	var quickReplies = ( cfg.quickReplies || [] ).filter(
		function ( q ) {
			return q && q.label && q.question; }
	);
	...
	function boldify( raw ) {
		...
		for ( var i = 0; i < lines.length; i++ ) {
			...
			var h = t.match( /^(#{1,6})\s+(.*)$/ );
			if ( h ) {
				flushPara(); closeList(); html += '<div class="nfx-md-h nfx-md-h' + Math.min( h[ 1 ].length, 4 ) + '">' + mdInline( h[ 2 ] ) + '</div>'; continue; }
			if ( /^>\s?/.test( t ) ) {
				flushPara(); closeList(); html += '<blockquote>' + mdInline( t.replace( /^>\s?/, '' ) ) + '</blockquote>'; continue; }
			var ul = t.match( /^[-*+]\s+(.*)$/ );
			if ( ul ) {
				flushPara(); if ( listType !== 'ul' ) {
					closeList(); html += '<ul>'; listType = 'ul'; } html += '<li>' + mdInline( ul[ 1 ] ) + '</li>'; continue; }
			var ol = t.match( /^\d+[.)]\s+(.*)$/ );
			if ( ol ) {
				flushPara(); if ( listType !== 'ol' ) {
					closeList(); html += '<ol>'; listType = 'ol'; } html += '<li>' + mdInline( ol[ 1 ] ) + '</li>'; continue; }
			closeList();
			para.push( t );
		}
		flushPara();
	...
	function applyColors() {
		if ( cfg.primaryColor ) {
			root.style.setProperty( '--nfx-primary', cfg.primaryColor ); }
		if ( cfg.primaryHover ) {
			root.style.setProperty( '--nfx-primary-hover', cfg.primaryHover ); }
		if ( cfg.buttonSize ) {
			root.style.setProperty( '--nfx-btn-size', parseInt( cfg.buttonSize, 10 ) + 'px' ); }
		if ( cfg.iconSize ) {
			root.style.setProperty( '--nfx-icon-size', parseInt( cfg.iconSize, 10 ) + 'px' ); }
		if ( cfg.buttonRadius != null && cfg.buttonRadius !== '' ) {
			root.style.setProperty( '--nfx-btn-radius', parseInt( cfg.buttonRadius, 10 ) + '%' ); }
	}
	...
	toggle.addEventListener(
		...
		function () {
			...
			if ( state.isOpen ) {
				if ( state.proactiveDismiss ) {
					state.proactiveDismiss(); }
				if ( ! state.started ) {
					startConversation(); }
				renderWindow();
			}
	...
	function closeChat() {
		...
		if ( voiceOut ) {
			try {
				window.speechSynthesis.cancel(); resetSpeakBtn(); } catch ( e ) {
				} }
	}
	...
	( function setupProactive() {
		if ( ! cfg.proactiveEnabled ) {
			return; }
		try {
			if ( sessionStorage.getItem( 'nfx_proactive' ) === 'dismissed' ) {
				return; } } catch ( e ) {
				}
				var shown = false, autoTimer = null, outsideHandler = null;
				function dismiss() {
					try {
						sessionStorage.setItem( 'nfx_proactive', 'dismissed' ); } catch ( e ) {
						}
						if ( autoTimer ) {
							clearTimeout( autoTimer ); autoTimer = null; }
						if ( outsideHandler ) {
							document.removeEventListener( 'click', outsideHandler, true ); outsideHandler = null; }
				...
				function showTeaser() {
					...
					outsideHandler = function ( e ) {
						if ( state.proactiveEl && ! state.proactiveEl.contains( e.target ) ) {
							dismiss(); }
					};
					setTimeout(
						function () {
	...
	function setProductQuickReplies() {
		if ( ! cfg.quickRepliesEnabled ) {
			state.chips = []; return; }
		var prod  = productById( state.selectedProduct );
		var chips = quickReplies.map(
			function ( qr ) {
				return chip(
					'pill',
					'',
		...
		if ( prod && prod.brochure ) {
			chips.push(
				chip(
					'brochure',
					ICON.fileText( 13 ),
					cfg.brochureLabel || 'مشاهده بروشور',
					function () {
	...
	function offerHandoff() {
		if ( cfg.handoffText ) {
			pushBot( cfg.handoffText, { noHistory: true } ); }
		state.chips = [ chip(
			'purple',
			ICON.headphones( 18 ),
			t( 'handoffBtn', 'گفتگو با کارشناس انسانی' ),
			function () {
				chooseConsult(); }
		) ];
	}
	...
	function hadRealConversation() {
		return state.items.some(
			function ( it ) {
				return it.kind === 'user' && ! it.noHistory; }
		);
	}
	...
	function sendMessage( text ) {
		if ( ! text || ! text.trim() || state.isLoading ) {
			return; }
		...
		var history = state.items
			.filter(
				function ( it ) {
					return ( it.kind === 'bot' || it.kind === 'user' ) && ! it.noHistory; }
			)
			.map(
				function ( it ) {
					return { role: it.kind === 'bot' ? 'assistant' : 'user', content : it.content }; }
			);
		...
		ajax(
			...
			function ( res ) {
				...
				if ( logId && cfg.feedbackEnabled ) {
					botItem.logId = logId; }
				if ( cfg.typewriter ) {
					botItem.animate = true; state.pendingAnimate = botItem; }
	...
	function submitForm() {
		...
		ajax( 'nafas_chatbot_submit', payload ).then(
			function ( res ) {
				state.isLoading = false;
				if ( res.ok && res.json && res.json.success ) {
	...
	function renderWindow() {
		...
		state.items.forEach(
			function ( it ) {
				thread.appendChild( renderItem( it ) ); }
		);

		if ( state.isLoading ) {
			thread.appendChild( typingEl() ); }
		if ( state.chips.length && ! state.isLoading ) {
			thread.appendChild( renderChips( state.chips ) ); }
	...
	function botActions( it ) {
		...
		if ( ! hasFeedback && ! hasSpeak && ! hasAnswer ) {
			return null; }
		...
		if ( hasAnswer ) {
			if ( wrap.children.length ) {
				wrap.appendChild( el( 'span', 'nfx-act-sep' ) ); }
	...
	function copyAnswer( it, btn ) {
		...
		function done() {
			...
			setTimeout(
				function () {
					btn.classList.remove( 'is-done' ); btn.innerHTML = ICON.copy( 15 ); },
				1500
			);
		}
	...
	function urgentAdr() {
		...
		if ( pid && pid !== companyInfo.id && productById( pid ) ) {
			openAdrForm( pid ); } else {
			chooseAdr(); }
	}
	...
	function loadVoices() {
		try {
			ttsVoices = window.speechSynthesis.getVoices() || []; } catch ( e ) {
			ttsVoices = []; }
	}
	if ( voiceOut ) {
		loadVoices();
		try {
			window.speechSynthesis.addEventListener( 'voiceschanged', loadVoices ); } catch ( e ) {
			try {
				window.speechSynthesis.onvoiceschanged = loadVoices; } catch ( e2 ) {
				}
			}
	}
	function pickVoice() {
		if ( ! ttsVoices.length ) {
			loadVoices(); }
		var fa = ttsVoices.filter(
			function ( v ) {
				return /fa|persian|farsi/i.test( ( v.lang || '' ) + ' ' + ( v.name || '' ) ); }
		)[ 0 ];
		if ( fa ) {
			return fa; }
		// در نبود صدای فارسی، عربی نزدیک‌ترین گزینه است؛ وگرنه هر صدای موجود.
		var ar = ttsVoices.filter(
			function ( v ) {
	...
	function maybeTypewriter() {
		...
		if ( ! bubble ) {
			state.pendingAnimate = null; return; }
	...
	function renderProductCard( p, it ) {
		...
		if ( p.summary ) {
			html += '<div class="nfx-prodcard__sum">' + escapeHtml( p.summary ) + '</div>'; }
		html           += '</div>';
	...
	function renderFormCard( kind ) {
		...
		ta.addEventListener(
			'input',
			function () {
				state.form.description = ta.value; }
		);
	...
	function buildFooter() {
		...
		function chooseAc( q ) {
			hideAc(); input.value = ''; sendMessage( q ); }
		...
		if ( ! state.isLoading && ! state.chips.length && ! hasForm && state.items.length && 'csat' !== lastKind && 'success' !== lastKind ) {
			setTimeout(
				function () {
					try {
						input.focus( { preventScroll: true } ); } catch ( e ) {
						input.focus(); } },
				60
			);
		}
